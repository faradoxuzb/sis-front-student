...
import { BaseService } from '../../../core/services/baseHttp.service';
import  moment from 'moment';
@Injectable({
    providedIn: 'root'
})
export class CalendarService
{
     ...
     * Constructor
     */
    constructor(private _httpClient: HttpClient, private _service: BaseService)
    {
    }
    ...
    {
        return this._service.get<any>('settings/core-types', {
            queryParams: {
                type_code: 'calendar-types'
            }
        }).pipe(
            tap((response) => {
                this._calendars.next(response.data.map(item => {
                    return {
                        id: item.value,
                        title: item.name['uz'],
    ...
    getEvents(start: Moment, end: Moment, replace = false): Observable<CalendarEvent[]>
    {
        ...
        // Get the events
        return this._service.get<any[]>('students/lesson-schedule', {
            queryParams: {
            ...
            map((response) => {
                return response.map(item => {
                    return {
                        id: item.id + '',
                        calendarId: 'calendar-type-01',
                        title: item.subject.name['uz'],
                        description: 'test-desc',
                        start: moment(item.date + ' ' + item.start_time, 'YYYY-MM-DD HH:mm:ss').toISOString() ,
                        end: moment(item.date + ' ' + item.end_time, 'YYYY-MM-DD HH:mm:ss').toISOString(),
                    } as CalendarEvent;
                });
            }),
